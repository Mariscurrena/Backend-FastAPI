...
async def user(user: User):
    ...
    id = db_client.users.insert_one(user_dict).inserted_id #Mongo instance - Schema, add inserted id

    new_user = user_schema(db_client.users.find_one({"_id":id})) # Looks for new user (id) in the DB, and returns a JSONS
    #ID in mongo is "_id
...
async def user(user: User):
    ...
    try:
        db_client.users.find_one_and_replace({"_id": ObjectId(user.id)}, user_dict) # Save user_dict, new parameter from user
    except:
...
def search_user(field: str, key):  ## Performing a generic function to be able to reuse it
    try: 
        user = user_schema(db_client.users.find_one({field: key})) # Perform transformation based on search criteria
